...
import { setToken, getToken, clearToken } from "./token-store.js";
...
async function main(): Promise<void> {
  try {
    ...
    if (!command || command === "--help" || command === "-h") {
      ...
      exitCode = args[0] ? await runCommand(args[0], { safe: args.includes("--safe") }) : missingFile();
      ...
      if (args[0] === "--clear") {
        ...
        if (currentToken) {
          console.log("Token is set (hidden for security)");
        } else {
          console.log("No token is set. Use: newt token <your-bot-token>");
        }
      }
    } else if (command === "new") {
